...
// RUTA PRA OBTENER LAS VENTAS EN UN RANGO DE FECHAS
router.get('/ventas', (req, res) => {
    ...
    if (fechaInicio > fechaFin) {
        return res.status(400).send('la fecha de inicio no puede ser posterior a la fecha final')
    }
...
router.post('/ventas', (req, res) => {
    ...
    // SEPARAR LOS PRODUCTOS Y EL TOTAL DE LA VENTA (ASUMIENDO QUE VENTA VIENE COMO UN STRING)
    ...
    db.query(query, [id_venta, productos, total_venta, fecha_venta, vendedor], (err, result) => {
        if (err) {
            console.error(err)
